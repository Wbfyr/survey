...
In the memory management section of this survey, we explore two components: the Key-Value (KV) cache and the long-term memory module. The former represents the most prominent source of memory overhead in LLMs, for which we investigate three classical optimization methods: PagedAttention, Tree Attention and Token Attention. The latter incurs additional memory overhead as context grows, and we discuss it through the lens of a Self-Control Memory model. Common memory optimization techniques in large models, such as model compression and distributed storage, while not unique to LLMs, are not elaborated upon in this survey.
...
In LLMs, memory resources are allocated for the computation of each token. However, not all tokens are actively used at all times. The memory management for each token in LLMs is not sufficiently detailed, leading to unnecessary occupation of memory resources, which can affect the overall performance and efficiency of the system.
...
Figure 4 depicts the system architecture and the overall workflow of Orca using the iteration-level scheduling.

\begin{figure}[htbp]
    \centerline{\includegraphics[width=0.5\textwidth]{process fig3.png}}
...
During the completion of this survey report, we sincerely thank our teacher for their guidance and support. Your expertise and encouragement have been invaluable in deepening our understanding of the survey's topics and methods.

We also appreciate assistants for their assistance. Your patient responses and guidance have helped us better grasp the survey's procedures and outcomes.

Our heartfelt thanks to all mentioned above!
